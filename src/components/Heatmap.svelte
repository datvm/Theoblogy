					...
					{#if day.notes.length > 0}
						...
						<ul class="flex flex-col gap-0.5">
							{#each day.notes as note}
								<a href={getRelativeLocaleUrl(locale, `/christianity/${monolocale ? note.id : note.id.split("/").slice(1).join("/")}`)} aria-label={note.data.title} class="ms-1 link">{note.data.title}</a>
							{/each}
						</ul>
					{/if}
					{#if day.jottings.length > 0}
						...
						<ul class="flex flex-col gap-0.5">
							{#each day.jottings as jotting}
								<a href={getRelativeLocaleUrl(locale, `/technology/${monolocale ? jotting.id : jotting.id.split("/").slice(1).join("/")}`)} aria-label={jotting.data.title} class="ms-1 link">{jotting.data.title}</a>
							{/each}
						</ul>
					{/if}
				{:else}
