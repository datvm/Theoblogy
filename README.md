...
## Theoblogy setup (this repo)

- `astro.config.ts` is set to `site: https://datvm.github.io/Theoblogy` with `base: /Theoblogy` for GitHub Pages project hosting.
- Update `site.config.ts` to tune title, bio, and social links.
- Local dev: `pnpm install` then `pnpm dev` (requires Node >= 20.17).
- Deploys via GitHub Actions workflow `.github/workflows/pages.yml` -> GitHub Pages.
